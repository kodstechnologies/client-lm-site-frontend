        ...
        <>
            {blogs.slice(pageVisited, pageVisited + blogsPerPage).map((data) => (
                <div className="blog-grid" key={data.id}>
                    <h3 className="title">
                        <Link to={PUBLIC_URL + `/blog-details/${data.id}`}>{data.title}</Link>
                    </h3>
                    <div className="author">
                        ...
                        <div className="info">
                            <h6 className="author-name">
                                <Link to={PUBLIC_URL + `/archive/${slugify(data.author_name)}`}>{data.author_name}</Link>
                            </h6>
                            ...
                            <Slider {...slideSettings} className="slick-arrow-nav">
                                {data.large_thumb.map((data, index) => (
                                    <div className="slide-item" key={index}>
                                        <img src={`${PUBLIC_URL}/${data}`} alt="Blog" />
                                    </div>
                                ))}
                                
                            </Slider> 
                            : <Link to={PUBLIC_URL + `/blog-details/${data.id}`}><img src={`${PUBLIC_URL}/${data.large_thumb}`} alt="Blog" /></Link>
                        }
                    ...
                    <p>{data.excerpt}</p>
                    <Link className="axil-btn btn-borderd btn-large" to={PUBLIC_URL + `/blog-details/${data.id}`}>Read More</Link>
                </div>
            ))}
